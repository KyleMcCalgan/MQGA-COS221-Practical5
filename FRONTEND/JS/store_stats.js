document.addEventListener('DOMContentLoaded', function(){
    ...
    function displayStoreStats(storeAverages){
        ...
        storeAverages.forEach(store =>{
            ...
            storeNameP.textContent = store.store_name || 'Unknown Store';

            const storeAvgPriceP = document.createElement('p');
            storeAvgPriceP.className = 'store-item-avg-price';
            storeAvgPriceP.innerHTML = `<strong>Avg. Price:</strong> ${store.average_store_price ? 'R' + store.average_store_price : 'N/A'}`;
            
            const storeAvgRatingP = document.createElement('p');
            storeAvgRatingP.className = 'store-item-avg-rating';
            storeAvgRatingP.innerHTML = `<strong>Avg. Rating:</strong> ${store.average_store_rating ? store.average_store_rating + '/5 ⭐' : 'N/A'}`; 
