...
if (!function_exists('handleGetAllListedProducts')) {
    function handleGetAllListedProducts($data, $db) {
        ...
        if (!$stmt) {
            error_log("Database prepare statement failed (get listed products - select user): " . $db->error);
            apiResponse(false, null, 'An internal error occurred. Please try again later.', 500);
        }
        $stmt->bind_param("s", $apiKey);
        if (!$stmt->execute()) {
            error_log("Database execute failed (get listed products - select user): " . $stmt->error);
            apiResponse(false, null, 'An internal error occurred. Please try again later.', 500);
        }
